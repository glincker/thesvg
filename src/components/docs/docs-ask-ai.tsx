...
const AI_TARGETS: AiTarget[] = [
  { label: "ChatGPT", buildUrl: (q) => `https://chatgpt.com/?q=${encodeURIComponent(q)}` },
  { label: "Claude", buildUrl: (q) => `https://claude.ai/new?q=${encodeURIComponent(q)}` },
  { label: "Perplexity", buildUrl: (q) => `https://www.perplexity.ai/search?q=${encodeURIComponent(q)}` },
];
...
export function DocsAskAi({ pageTitle }: { pageTitle: string }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 z-20 mt-1.5 w-40 overflow-hidden rounded-xl border border-border/60 bg-background shadow-lg dark:border-white/[0.1] dark:bg-[#0f0f10]">
