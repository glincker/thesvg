 ...
 * `thesvg@<version>`, taken from packages/thesvg/package.json's own version
 * field at publish time. src/data/thesvg-version.json mirrors that field
 * (regenerated by generate-llms-txt.ts on every build) so the jsDelivr pin
 * example below tracks whatever was actually last released instead of a
 * hardcoded tag that goes stale the next time a version bumps - as a plain
 * JSON import rather than a readFileSync call, it also stays safe to import
 * from client-side code (e.g. the header search index), unlike Node's `fs`.
 */
