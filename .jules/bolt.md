...
## 2025-02-18 - Replacing Array.prototype.find and Higher-Order Array Iterations for Icons Data
**Learning:** In `src/lib/icons.ts`, large icons array manipulations for UI filtering and layout lookups can introduce continuous iteration and closure initializations when using `.filter().some()` chains and O(N) array searching like `.find()`. In Next.js SSR and client renders, these are on the hot path.
**Action:** Always pre-compute a `Map` lazily for O(1) lookups by unique identifiers (like `slug`) across large static datasets, and replace nested higher-order array iterations (`.filter`, `.some`) with flat `for` loops inside data querying modules to skip overhead.
