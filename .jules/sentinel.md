...
## 2025-02-14 - Replace Regex SVG Validation with DOMParser to Fix XSS Evasion
**Vulnerability:** The function `validateSvg` in `src/lib/svg-validation.ts` relied on regex (e.g., `/href\s*=\s*["']javascript:/i.test(content)`) on raw SVG strings to prevent embedded scripts.
**Learning:** Regex string checks are easily bypassed by techniques like HTML entity encoding (e.g. `&#x6A;avascript:`). The browser's `DOMParser` decodes these entities. By traversing the DOM generated by `DOMParser` instead of the raw string, entity-encoded XSS payloads can be accurately detected.
**Prevention:** Rely on DOM parsing logic (`DOMParser`) to walk elements and check `el.attributes` instead of regex string replacements or tests when checking for malicious attributes or URIs.
