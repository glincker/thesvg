...
## 2025-02-14 - XSS in Markdown Protocol Links and Headers
**Vulnerability:** Markdown links in the blog renderer (`src/app/blog/[slug]/page.tsx`) were using a blocklist to prevent `javascript:` URIs, which could be bypassed with `data:text/html` or similar schemes. Furthermore, markdown headers parsed via slicing were not HTML-escaped before being rendered.
**Learning:** Always use a strict allowlist for URLs (`http:`, `https:`, relative paths, anchors) rather than a blocklist to prevent scheme-based XSS. Also, all user-derived components of a manually parsed string must be HTML-escaped before insertion into `dangerouslySetInnerHTML`.
**Prevention:** Rely on explicit allowlists for allowed URL protocols. Explicitly escape text parsed directly from un-escaped content loops (such as markdown AST equivalents) before emitting raw HTML.
